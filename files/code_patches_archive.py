...
#%% Prepare full rank data5 J=3 classes 18ktr, with rangdom AOA, 1000 val, 1000 te
    from utils import *
    plt.rcParams['figure.dpi'] = 150
    torch.set_printoptions(linewidth=160)
    ...
    plt.imshow(x[0,0].abs(), aspect='auto', interpolation='None')
    plt.title('One example of 3-component mixture')
    torch.save(x[:18000], f'tr18kM3FT{FT}_data5.pt')
    ...
    valtest = awgn_batch(valtest, snr=40, seed=1) # added white noise
    svaltest = torch.tensor(np.stack(svaltest, axis=1))  #[2000, J, F, T]
    torch.save((valtest[:1000], svaltest[:1000], hall[9,:1000]), f'val1kM3FT{FT}_xsh_data5.pt')
    torch.save((valtest[1000:], svaltest[1000:], hall[9,1000:]), f'test1kM3FT{FT}_xsh_data5.pt')
    print('done')
