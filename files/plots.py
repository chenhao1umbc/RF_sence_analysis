...
#%% EM, VAE 6-classes
    ...
    em = [0.7445756667554378]*5
    # vae = [0.6853250654935836,  0.7350429352819919, 0.7588296358287334, 0.7619795973300933, 0.7876128880381584, 0.8040]#, 0.8568]
    # nem = [0.7686689290784765,0.770921271815709, 0.7710523081942222, 0.7756224503521928, 0.7829264613947694, 0.7863359329]#, 0.7186788338164556]
    vae = [0.6853250654935836,  0.7350429352819919, 0.7588296358287334, 0.7876128880381584, 0.81074, 0.8568]
    nem = [0.7686689290784765, 0.770921271815709, 0.7710523081942222, 0.7829264613947694, 0.7863359329]#, 0.7186788338164556]    
    plt.figure()
    plt.plot(torch.tensor([1e3, 2e3, 3e3, 6e3, 12e3]).log(), em, '-', color="#1f77b4") 
    plt.plot(torch.tensor([1e3, 2e3, 3e3, 6e3, 12e3]).log(), nem, '--o', color="#2ca02c") 
    plt.plot(torch.tensor([1e3, 2e3, 3e3, 6e3, 12e3, 18e3]).log(), vae, '-.v', color="#ff7f0e") 
    plt.xticks(torch.tensor([1e3, 2e3, 3e3, 6e3, 12e3, 18e3]).log(),['1k', '2k', '3k', '6k', '12k', '18k'])
    plt.legend(['EM', 'NEM', 'VAE'])
    plt.ylabel('STFT correlation')
