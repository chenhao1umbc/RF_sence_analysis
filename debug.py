...
def dist(data, c1, c2):
    """calc the distance between two clusters. There are many ways to achieve it, on this webpage
    https://vaghefi.medium.com/fast-distance-calculation-in-python-bb2bc9810ea5
    it shows the benchmark of each method. here we use scipy_cdist
    ...
    # for i1 in l1:
    #     for i2 in l2:
    #         res = res + ((data[i1]-data[i2]).abs()**2).sum(dim=-1)**0.5
    res = (((data[l1,None,:]-data[None,l2]).abs()**2).sum(-1)**0.5).sum()
...
dn = dendrogram(z, p=6, truncate_mode='level')
