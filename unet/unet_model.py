...
class SBD3(nn.Module):
    """This is spatial broadcast decoder (SBD) version
    Input shape [I, d_gamma], e.g.[32,64]"""
    def __init__(self, dz=32, im_size=100, max_ch=16):
        super().__init__()
        self.decoder = nn.Sequential(
            SingleConv(in_channels=dz+2, out_channels=max_ch),
            SingleConv(in_channels=max_ch, out_channels=max_ch),
            SingleConv(in_channels=max_ch, out_channels=max_ch//2),
            OutConv(max_ch//2, 1),
            nn.Sigmoid()
            ) 
