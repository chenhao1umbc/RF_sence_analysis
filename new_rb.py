...
class NNet_s10(nn.Module):
    ...
    def forward(self, x):     
        ...
        rb = (xj@xj.conj().permute(0,1,2,4,3)).mean(dim=(1,2)).squeeze().abs()
        delta = threshold(rb.diagonal(dim1=-1, dim2=-2).mean(-1), floor=1e-3, ceiling=100)
        eye = torch.eye(M, device='cuda')
        Rb = torch.stack(tuple(eye for ii in range(btsize)), 0)*delta[:,None,None]
...
xval, sval, hgt = torch.load('../data/nem_ss/val1kM6FT64_xsh_data4.pt')
...
model = torch.load('../data/data_ss/models/s131_a/model_epoch640.pt')
...
hall, sall = [], []
with torch.no_grad():
    for ii in range(0,701,5):
        model = torch.load(f'../data/data_ss/models/s131_a/model_epoch{ii}.pt')
        for snr in [0]:
            av_hcorr, av_scorr = [], []
