...
def nem_hci(x, J=6, Hscale=1, Rbscale=1, max_iter=501, seed=1, model=''):
    ...
    # g = awgn(g, snr=20).abs().to(torch.float).cuda()
    # g = torch.rand(1,J,1,8,8, dtype=torch.float)
    noise = torch.load('../data/nem_ss/lb_c6_J188.pt') # shape of [J,1,8,8], cpu()
    g = (g + noise/100).to(torch.float).cuda() 
    x = x.cuda()
...
for db in [30,20,10,5,0]:
    ...
    for ii in range(100):
        ...
        rid = 184110
        model = f'../data/nem_ss/models/rid{rid}/model_rid{rid}_24.pt'
